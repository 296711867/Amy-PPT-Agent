...
const enforceGenerationSchema = async (client: LibSqlClient): Promise<void> => {
  ...
  if (!columns.has('image_asset_path')) {
    await client.execute('ALTER TABLE generation_pages ADD COLUMN image_asset_path TEXT')
  }
  if (!columns.has('image_asset_paths')) {
    await client.execute('ALTER TABLE generation_pages ADD COLUMN image_asset_paths TEXT')
  }
  await client.execute(
    'CREATE INDEX IF NOT EXISTS idx_generation_runs_session ON generation_runs(session_id, created_at)'
  )
...
const enforceSourcePageSkeletonsSchema = async (client: LibSqlClient): Promise<void> => {
  const columns = await getTableColumns(client, 'source_page_skeletons')
  if (!columns.has('layout_intent')) {
    await client.execute('ALTER TABLE source_page_skeletons ADD COLUMN layout_intent TEXT')
  }
  if (!columns.has('layout_id')) {
    await client.execute('ALTER TABLE source_page_skeletons ADD COLUMN layout_id TEXT')
  }
}
