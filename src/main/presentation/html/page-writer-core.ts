...
function stripCanvasInlineSizes(styleAttr: string): string {
  ...
  const kept = declarations.filter((decl) => {
    ...
    if (
      new RegExp(`^(width|min-width|max-width): (${dimensionValuePattern}|100vw|100dvw)$`).test(
        normalized
      )
    )
      return false
    if (
      new RegExp(`^(height|min-height|max-height): (${dimensionValuePattern}|100vh|100dvh)$`).test(
        normalized
      )
    )
      return false
...
async function buildScaffoldDocument(args: {
  ...
  const fontInjection = designFonts
...
export async function persistPageHtmlFromFragment(args: {
  ...
  validateDeckConsistency?: () => Promise<{ details: string[]; message: string } | null>
}): Promise<{
  html: string
  content: string
  repaired: boolean
  originalErrors?: string[]
  qualityWarnings: QualityViolation[]
}> {
  const persistedRaw = await buildPersistedPageHtmlFromFragment(args)
  ...
  if (harnessErrors.length > 0) {
    throw new PageWriteValidationError(
      'harness-quality',
      args.pageId,
      harnessErrors.map((v) => `${v.code}: ${v.detail}`),
      [
        `质量校验未通过 (${args.pageId})：按下列每一条逐条修正后重新调用写盘工具，不要辩解、不要绕过。`,
        ...harnessErrors.map((v) => `- [${v.code}] ${v.detail} → ${v.fix}`)
      ].join('\n')
    )
  }
  ...
  await serializedWrite(args.projectDir, async () => {
    ...
    const restorePreviousHtml = async (): Promise<void> => {
      if (previousHtml === null) {
        await fs.promises.unlink(args.targetPath).catch((error) => {
          const code =
            error && typeof error === 'object' ? (error as NodeJS.ErrnoException).code : undefined
          if (code !== 'ENOENT') throw error
        })
      } else {
        await fs.promises.writeFile(args.targetPath, previousHtml, 'utf-8')
      }
    }
    await fs.promises.writeFile(args.targetPath, persisted.html, 'utf-8')
    if (args.validateRenderedPage) {
      let rendered: RenderedPageValidationResult
      try {
        rendered = await args.validateRenderedPage({
          pageId: args.pageId,
      ...
      if (!rendered.available) {
        ...
        qualityWarnings.push({
          ...
          severity: 'warn',
          detail: `浏览器渲染验收暂不可用：${rendered.unavailableReason || 'unknown renderer failure'}`,
          fix: '页面已通过静态规则并保留；完成后请在预览中人工检查文字越界、裁切和重叠'
        })
      } else {
        const renderedErrors = rendered.violations.filter(
          (violation) => violation.severity === 'error'
        )
        qualityWarnings.push(
          ...rendered.violations.filter((violation) => violation.severity === 'warn')
        )
        if (renderedErrors.length > 0) {
          ...
          throw new PageWriteValidationError(
            'rendered-quality',
            args.pageId,
            renderedErrors.map((violation) => `${violation.code}: ${violation.detail}`),
            [
              `浏览器渲染校验未通过 (${args.pageId})，已恢复写入前页面。请逐条修正后重新调用写盘工具。`,
              ...renderedErrors.map(
                (violation) => `- [${violation.code}] ${violation.detail} → ${violation.fix}`
              )
            ].join('\n')
          )
        }
    ...
    const deckFailure = await args.validateDeckConsistency?.()
    if (deckFailure) {
      await restorePreviousHtml()
      throw new PageWriteValidationError(
        'deck-quality',
        args.pageId,
        deckFailure.details,
        `${deckFailure.message}\n已恢复写入前页面，请按 Deck 反馈修正后重试。`
      )
    }
  })
  return { ...persisted, qualityWarnings }
}

/** Validate a full page after a selector-level edit without rebuilding its runtime shell. */
export async function validatePersistedPageAfterEdit(args: {
  pageId: string
  targetPath: string
  slideSize: SlideSizePreset
  validateRenderedPage?: (args: {
  ...
  if (!persistedValidation.valid) {
    throw new PageWriteValidationError(
      ...
      args.pageId,
      persistedValidation.errors,
      `Selector 编辑后的 HTML 落盘校验失败 (${args.pageId}): ${persistedValidation.errors.join('; ')}。已恢复编辑前页面，请缩小修改范围后重试。`
    )
  }

  const qualityViolations = validatePageQuality(html, args.slideSize)
  const qualityErrors = qualityViolations.filter((violation) => violation.severity === 'error')
  const qualityWarnings = qualityViolations.filter((violation) => violation.severity === 'warn')
  if (qualityErrors.length > 0) {
    throw new PageWriteValidationError(
      'harness-quality',
      args.pageId,
      qualityErrors.map((violation) => `${violation.code}: ${violation.detail}`),
      [
        `Selector 编辑后的质量校验未通过 (${args.pageId})，已恢复编辑前页面。请逐条修正后重新调用 edit_file。`,
        ...qualityErrors.map(
          (violation) => `- [${violation.code}] ${violation.detail} → ${violation.fix}`
        )
      ].join('\n')
  ...
  if (!args.validateRenderedPage) return qualityWarnings
  let rendered: RenderedPageValidationResult
  try {
    rendered = await args.validateRenderedPage({
      pageId: args.pageId,
  ...
  if (!rendered.available) {
    qualityWarnings.push({
      ...
      severity: 'warn',
      detail: `浏览器渲染验收暂不可用：${rendered.unavailableReason || 'unknown renderer failure'}`,
      fix: 'Selector 修改已通过静态规则并保留；请在预览中人工检查文字越界、裁切和重叠'
    })
    return qualityWarnings
  }

  const renderedErrors = rendered.violations.filter((violation) => violation.severity === 'error')
  qualityWarnings.push(...rendered.violations.filter((violation) => violation.severity === 'warn'))
  if (renderedErrors.length > 0) {
    throw new PageWriteValidationError(
      'rendered-quality',
      args.pageId,
      renderedErrors.map((violation) => `${violation.code}: ${violation.detail}`),
      [
        `Selector 编辑后的浏览器渲染校验未通过 (${args.pageId})，已恢复编辑前页面。请逐条修正后重新调用 edit_file。`,
        ...renderedErrors.map(
          (violation) => `- [${violation.code}] ${violation.detail} → ${violation.fix}`
        )
      ].join('\n')
