...
(async () => {
  ...
  const root =
    ...
    document.body;
  const existing = document.getElementById('amy-ppt-export-freeze-page');
  if (existing) existing.remove();
  const style = document.createElement('style');
  style.id = 'amy-ppt-export-freeze-page';
  style.textContent = [
...
(async () => {
  const existing = document.getElementById('amy-ppt-pptx-hide-text');
  if (existing) existing.remove();
  ...
  const style = document.createElement('style');
  style.id = 'amy-ppt-pptx-hide-text';
  style.textContent = [
...
(async () => {
  ...
  const style = document.createElement('style');
  style.id = 'amy-ppt-pptx-hide-elements';
  style.textContent = [
...
(async () => {
  document.getElementById('amy-ppt-pptx-hide-elements')?.remove();
  void document.body.offsetHeight;
  await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
  return true;
})()
...
(async () => {
  let existing = document.getElementById('amy-ppt-pptx-hide-elements');
  if (existing) existing.remove();
  const style = document.createElement('style');
  style.id = 'amy-ppt-pptx-hide-elements';
  style.textContent = [
