...
import type { SessionPageRecord } from '../db/records'
...
export async function finalizeGenerationFailure(
  ...
  if (!cancelled && failure.scope === 'system') {
    const sessionPages = await db.listSessionPages(context.sessionId)
    const completedPageCount = sessionPages.filter((page) => page.status === 'completed').length
    const pendingPages = sessionPages.filter((page) => page.status !== 'completed')
    if (generationRun && generationRun.status === 'running') {
      await db.updateGenerationRunStatus(
        context.runId,
        completedPageCount > 0 ? 'partial' : 'failed',
        failure.technicalDetail
      )
    }
    await db.updateSessionStatus(
      context.sessionId,
      completedPageCount > 0 ? 'active' : resolveGenerationFailureSessionStatus(context, false)
    )
    await db.addMessage(context.sessionId, {
      role: 'system',
      content: `${failure.titleZh}：${failure.detailZh}\n${failure.technicalDetail}`,
      type: 'stream_chunk',
    ...
    ctx.runtimeEmitters.emitGenerateChunk(context.sessionId, {
      ...
      payload: {
        runId: context.runId,
        message: `${failure.titleZh}：${failure.detailZh}`,
        failure,
        completedPageCount,
        failedPageCount: sessionPages.filter((page) => page.status === 'failed').length,
        pendingPageCount: pendingPages.length,
        pendingPageIds: pendingPages.map((page) => page.file_slug),
        occurrences: 1,
        provider: 'provider' in context ? String(context.provider || '') : undefined,
        model: 'model' in context ? String(context.model || '') : context.runModel
      }
    })
    return
  }
