...
const buildPageEditAssessmentSystemPrompt = (
  locale: 'zh' | 'en',
  args: {
    targetPageId: string
    targetPageNumber?: number
  ...
  const selectorContext = [
    ...
    args.elementTag
      ? `Element: <${args.elementTag}>${args.elementText ? ` ${args.elementText}` : ''}`
      : '',
...
export async function assessPageEdit(
  ...
  const page = pages.find(
    (item) => item.id === requestedPageId || item.file_slug === requestedPageId
  )
  ...
  const imagePrompt = ctx.localFiles.formatImagePathsForPrompt(
    input.rawImagePaths,
    input.rawVideoPaths
  )
