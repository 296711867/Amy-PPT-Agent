...
export const isStructuralFragmentValidationError = (detail: string): boolean =>
  /HTML 末尾存在未闭合标签|开闭标签数量不一致|闭标签多于开标签|缺少结尾|缺少 <\/body>/i.test(detail)
...
export const buildEditValidationRetryMessage = (
...
export async function generatePagesWithRetry(
  ...
  if (firstResult.pause) {
    throw new Error(
      `${firstResult.pause.failure.titleZh}：${firstResult.pause.failure.technicalDetail}`
    )
  }
  ...
  if (retryResult.pause) {
    throw new Error(
      `${retryResult.pause.failure.titleZh}：${retryResult.pause.failure.technicalDetail}`
    )
  }
