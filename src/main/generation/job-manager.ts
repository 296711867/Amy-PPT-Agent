...
export class GenerateJobManager {
  ...
  private async runJob(job: BackgroundJob<FinalizeContext>, activationError?: unknown): Promise<void> {
    try {
      ...
      const runState = this.ctx.sessionRuns.sessionRunStates.get(job.sessionId)
      const paused = runState?.runId === job.runId && runState.status === 'paused'
      this.ctx.runtimeEmitters.emitRuntimeJobTerminal({
        sessionId: job.sessionId,
        jobId: job.runId,
        domain: 'generation',
        status: paused ? 'failed' : 'completed',
        errorCode: paused ? 'generation_paused' : undefined,
        errorMessage: paused ? runState.error || 'Generation paused' : undefined
      })
    } finally {
