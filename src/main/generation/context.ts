...
export async function resolveCommonContext(
  ...
  const layoutRulesPrompt = buildLayoutRulesPrompt(
    normalizeLayoutRules(settings[LAYOUT_RULES_SETTING_KEY]),
    slideSize
  )
  ...
  return {
    ...
    deckTitle: String(sessionRecord.title || 'Amy-PPT Preview'),
    appLocale,
    ...
    visualElementPreferences: normalizeVisualElementPreferences(
      sessionMetadata.visualElementPreferences
    ),
