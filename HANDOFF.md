...
- **下一步优先级 2**：超大文件拆分告一段落——主进程/渲染进程可拆目标全部完成（第 31/33/34/35/36 项），剩余最大文件为 `db/database.ts`（2039，仓库约定的单一持久化门面）与 `db/patch/index.ts`（1646，历史迁移）与 i18n 词典（纯数据），均按约定不动；`document-parse-handlers.ts`（1471→363）已拆完。后续如需继续，优先做依赖审计（只生成候选逐个确认）。
