...
- **下一步优先级 2**：`db/patch/index.ts`（1646，历史迁移）、`renderer/lib/ipc.ts`（1666）、`document-parse-handlers.ts`（1471）是剩余最大的文件；行为改动必须配定向测试。`agent-runner`、`pptx-import`、`PreviewIframe` 已拆完（第 31/33/34 项）。
