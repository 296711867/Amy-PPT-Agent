...
- **下一步优先级 2**：依赖审计已闭环（第 39 项，零候选）。超大文件拆分也告一段落（第 31/33/34/35/36 项）；`db/database.ts`（单一持久化门面）、`db/patch/index.ts`（历史迁移）、i18n 词典按约定不动。可做的方向：给 `logs/main.log` 加轮转/清理策略（dev 下已 38 万字符）、把 deck-review-repair 的评审结果持久化到 session_events 便于 UI 展示、或继续真实运行观察标题带锚点与渲染验收的表现。
