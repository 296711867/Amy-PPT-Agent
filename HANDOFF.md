...
- **下一步优先级 2**：`document-parse-handlers.ts`（1471，文档解析：mammoth 转换/计划质量校验/页面摘要，io 域）是剩余最大可拆文件；行为改动必须配定向测试。`db/patch/index.ts`（1646）为历史迁移按约定不动。`agent-runner`、`pptx-import`、`PreviewIframe`、`renderer/lib/ipc` 已拆完（第 31/33/34/35 项）。
