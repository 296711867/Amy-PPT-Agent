---
name: amy-ppt-layout
description: Must be read before creating, relaying out, or repairing Amy-PPT slide layouts. Defines the per-page decision path, the named layout pattern catalog, canvas budgeting, collision avoidance, chart overpack guardrails, and title readability rules.
---
...
image policy: use existing assets; add a replaceable semantic placeholder only when the User PPT Composition Profile explicitly allows it and the composition truly requires an image
```
...
| `quote` | hero-quote |
| `summary` | summary-takeaways · executive-brief · conclusion-with-proof |
| `data-focus` | kpi-hero · metric-band · trend-exhibit · chart-annotated · big-number-evidence |
| `comparison` | compare-two-zone · compare-options · decision-matrix · comparison-axis |
| `concept` | concept-center-satellites · framework-2x2 · framework-pyramid · central-radiation |
| `process` | process-linear · process-loop · staircase-strips · diagonal-progression |
| `timeline` | timeline-strip |
| `image-focus` | asset-image-hero · asset-text-visual-split · image-led-story |
...
- Full-bleed backgrounds may use the entire 1600×900 canvas. Authored readable content must stay inside the safe area supplied by the User Layout Profile when present; otherwise keep a conservative baseline reserve. Only backgrounds, full-bleed images, textures, and decorative shapes may cross that safe area.
...
- Every heading must be at least `text-2xl` (24px); this is a floor, not a fixed heading size or maximum. Use the User Layout Profile typography targets when present, including larger explicit pixel sizes for cover titles and emphasis text.
...
- Keep slide subtitles and module titles semantically distinct. A slide subtitle or lead explains the whole page; a module second-level title names one step, stage, point, or evidence block. Follow the User PPT Composition Profile targets when present; otherwise keep module titles at or above the 24px heading floor.
...
**Active fill is a default duty, not an option.** 内容页与数据页必须读起来像一张被完整设计过的画面，而不是半空的网页。当源内容不足以让画面饱满——模块只挤在上半部分、下半部分是大片被遗忘的空白带、或一个内容区里只有一两个孤立小卡片——你必须主动在页内扩充，直到整页读起来是刻意的、充实的构图。半空的内容/数据页是缺陷，不是"极简"；极简只属于封面、章节扉页、金句页这类低密度页型（见下方豁免）。

写入前的充实度自检（必做）：扫一眼整体结构，如果内容明显只占了画布的上半部分或中间一块，底部或侧边留出大片无意的空白带——必须先扩充再落盘。扩充优先级：① **放大主体**（hero 数字 / 图表 / 图像占满它的 zone）→ ② **补一条 evidence rail**（1-2 张支撑卡片 / 指标 / 案例佐证）→ ③ **加视觉锚点**（图示 / 轴线 / 时间线 / 对比括号 / 象限 / 进度条）→ ④ **扩展论点**（背景 / 对比 / 基线 / 原因 / 启示 / "so what" 一句话）。宁可适度充实，也不要交一个半空的页面。系统层还有一道极保守的兜底：内容极少又没有任何主体视觉的孤页会被自动打回重写，所以不要心存侥幸地交半空页面。
...
Amy-PPT separates two concerns:
